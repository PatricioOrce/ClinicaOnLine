...
//               data.especialidades.forEach(x => {
//                 if(x.descripcion == this.especialidadSeleccionado.descripcion){
...
//     // // Filtrar los especialistas de acuerdo a la especialidad seleccionada
//     // this.especialistasEspecialidad = this.especialistas.filter(
//     //   (especialista: any) => {
//     //     return especialista.especialidades.some(
//     //       (especialidad: any) =>
//     //         especialidad.id === this.especialidadSeleccionado.id
//     //     );
//     //   }
//     // );
//   }
...
export class SolicitarTurnoComponent implements OnInit, OnDestroy {
  ...
  cargarEspecialistas() {
    var especialistasFiltered: Array<Especialista> = [];
    this.suscripciones.push(
      this.especialistaService.getAllTwo().subscribe((snapshot) => {
        this.especialistas = snapshot;
        console.log("Lookkk", snapshot)
        console.log("Lgdfgdfg", this.especialidadSeleccionado.nombre)
        snapshot.forEach(x => {
          if(x.especialidades){
            x.especialidades.forEach(y => {
              if(y.descripcion == this.especialidadSeleccionado.descripcion){
                especialistasFiltered.push(x);
              }
            })
          }
        })
        this.especialistas = especialistasFiltered;
      })
    );
  }
  cargarEspecialidades() {
  ...
  // cargarHorariosEspecialidad() {
  //   this.especialidadSeleccionado.nombre;
  //   console.log(this.especialistaSeleccionado.horarios);
  // }
  ...
  getDates() {
    ...
    for (var i = 0; i <= date_range; i++) {
      ...
      var fmt_date = {
        ...
        date: getDate,
        month: months[current_date.getMonth()],
        // Modificar la propiedad 'month' para mostrar el nombre del mes
  ...
  registrarTurno() {
    ...
    if (
      ...
      this.turno.especialista = this.especialistaSeleccionado;
      this.turno.especialidad = this.especialidadSeleccionado;
      if (this.authSvc.usuarioLogeado.roles.includes('admin')) {
        this.paciente = this.pacienteSeleccionado;
        this.turno.paciente = this.pacienteSeleccionado;
      } else if (this.authSvc.usuarioLogeado.roles.includes('paciente')) {
        this.paciente = this.authSvc.usuarioLogeado;
        this.turno.paciente = this.paciente;
      }
      // this.turno.fecha =
      ...
      this.turnoSvc
        ...
        .then((res: any) => {
          ...
          setTimeout(() => {
            if (this.authSvc.usuarioLogeado.perfil == 'paciente') {
              this.router.navigate(['/intranet/mis-turnos-paciente']);
            } else if (this.authSvc.usuarioLogeado.perfil == 'admin') {
              this.router.navigate(['/administracion/turnos']);
            }
          }, 2000);
          // this.router.navigate(['/intranet/solicitar-turno']);
